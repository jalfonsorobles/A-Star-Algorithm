...
bool verifyInput(char* string) {
    ...
    if (strlen(string) > 9) {
        printf("\nMore than 9 entries were entered. Try again:\n");
        return false;
    }
    
    if (strlen(string) < 9) {
        printf("\nLess than 9 entries were entered. Try again:\n");
        return false;
    }
    
    // iteriates to find numbers 1-8 and blank tile
    for (int i = 1; i < 9; i++) {
        
        for (int j = 0; j < 9; j++) {
            
            // converts char to int via ASCII values
            int y = ((int)string[j]) - 48;
            ...
            if (j == 8) {
                printf("\nMissing tile number %d. Try again:\n", i);
                return false;
            }
        }
    }
    
    // searches for correct blank tile
    for (int i = 0; i < 9; i++) {
        char c = string[i];
        ...
        if (c == '*' || c == '-' || c == '_') {
            ...
            printf("\nMissing correct blank tile. Use [_ , *, or -] as the blank tile and try again:\n");
            return false;
        }
    }
    return true;
...
void updateBlank(board* currState) {
    ...
    for (int i = 0; i < strlen(currState->string); i++) {
        ...
        if (currState->string[i] == '_' || currState->string[i] == '*' || currState->string[i] == '-') {
            currState->blank = i;
            break;
        }
    }
...
int numMisplaced(board* currState, board* goalState) {
    ...
    for (int i = 0; i < strlen(currState->string); i++) {
        ...
        if (currState->string[i] != goalState->string[i] && (currState->string[i] != '_' || currState->string[i] != '*' || currState->string[i] != '-')) {
            misplacedTiles++;
        }
    }
...
void moveTile(board* currState, int move, int depth) {
    ...
    if (move == 0) {
        ...
        // place the blank tile at new location (-1)
        currState->string[blank - 1] = currState->string[blank];
        // place the swapped char where the blank tile used to be
    ...
    // if move is UP
        ...
        // place the blank tile at new location (-3)
        currState->string[blank - 3] = currState->string[blank];
        // place the swapped char where the blank used to be
        currState->string[blank] = swap;
        // update blank
...
void updateFnManhattanDistance(board* currState, board* goalState) {
    ...
    int x1, y1, x2, y2 = 0;
        ...
        for (int i = 0; i < strlen(goalState->string); i++) {
            ...
            if (goalState->string[i] == '_' || goalState->string[i] == '*' || goalState->string[i] == '-') {
                continue;
            }
